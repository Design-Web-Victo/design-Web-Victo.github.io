...
Si vous avez un fichier au format SVG, vous pouvez simplement utiliser la balise `<img>` comme pour les autres types d'images.

```html
<img src="image.svg" alt="Illustration vectorielle">
```
...
Dans cette exemple on indique avec la balise `<circle>` de dessiner un cercle selon les attributs donnés. 

Pour plus de détail sur la création d'un SVG : [Tutoriel SVG avec HTML5](https://edutechwiki.unige.ch/fr/Tutoriel_SVG_avec_HTML5){target=_blank}. Vous n'avez pas besoin d'apprendre à créer des SVG de cette façon. Certains sites vous en offre gratuitement et vous n'avez qu'à copier le SVG désiré.

```html title="Exemple tiré du site fontawesome.com"
